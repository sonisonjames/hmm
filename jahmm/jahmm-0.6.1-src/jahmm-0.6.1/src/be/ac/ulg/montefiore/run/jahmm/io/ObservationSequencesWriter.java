...
public class ObservationSequencesWriter
{
	...
	write(Writer obsWriter, Writer seqWriter, ObservationWriter<O> ow,
			List<List<O>> sequences)
	throws IOException
	{
		for (List<O> obs : sequences)
			write(obs, ow, obsWriter, seqWriter);
	}
	...
	write(List<O> sequence, ObservationWriter<O> ow, 
			Writer obsWriter, Writer seqWriter) 
	throws IOException
	{
		for (O obs : sequence) 
			ow.write(obs, obsWriter, seqWriter);
