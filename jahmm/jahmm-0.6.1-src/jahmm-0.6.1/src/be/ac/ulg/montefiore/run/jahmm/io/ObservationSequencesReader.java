...
{	
	 ...
	 *               from.
	 * @return first {@link java.util.Vector Vector} of 
	 *         {@link java.util.Vector Vector}s of
	 *         {@link be.ac.ulg.montefiore.run.jahmm.Observation Observation}s that are used for hmm generation
	 *         second {@link java.util.Vector Vector} of 
	 *         {@link java.util.Vector Vector}s of
	 *         {@link be.ac.ulg.montefiore.run.jahmm.Observation Observation}s that are used to testing the prediction
	 */
	static public <O extends Observation> Pair<ArrayList<List<O>>, ArrayList<List<O>>>
	readSequences(ObservationReader<O> or, Reader reader, int learnSeqLen)
	throws IOException, FileFormatException
	{
		Pair<ArrayList<List<O>>,ArrayList<List<O>>> sequences =
				Pair.makePair(new ArrayList<List<O>>(), new ArrayList<List<O>>());
		...
		st.nextToken()) {
			st.pushBack();
			Pair<List<O>, List<O>> sequence = readSequence(or, st, learnSeqLen);
			
			if (sequence == null)
				break;
			
			sequences.first.add(sequence.first);
			sequences.second.add(sequence.second);
		}
	 ...
	 */
	static public <O extends Observation> Pair<List<O>, List<O>> 
	readSequence(ObservationReader<O> oir, Reader reader, int learnSeqLen) 
	throws IOException, FileFormatException
	{	
		StreamTokenizer st = new StreamTokenizer(reader);
		initSyntaxTable(st);
		
		return readSequence(oir, st, learnSeqLen);
	}
	 ...
	 * initialized.
	 */
	static <O extends Observation> Pair<List<O>, List<O>>
	readSequence(ObservationReader<O> oir, StreamTokenizer st, int learnSeqLen) 
	throws IOException, FileFormatException
	{	
		...
		List<O> sequence1 = new ArrayList<O>();
		
		do {
			st.pushBack();
			sequence1.add(oir.read(st));
			...
			if (st.nextToken() == '\\') { /* New lines can be escaped by '\' */
				...
				sequence1.size() < learnSeqLen);
		
		if (st.ttype == StreamTokenizer.TT_EOF)
			throw new FileFormatException("Unexpected token: EOF"); 
		
		// read till EOL
		List<O> sequence2 = new ArrayList<O>();
		
		do {
			st.pushBack();
			sequence2.add(oir.read(st));
		...
		return Pair.makePair(sequence1, sequence2);
	}
}
