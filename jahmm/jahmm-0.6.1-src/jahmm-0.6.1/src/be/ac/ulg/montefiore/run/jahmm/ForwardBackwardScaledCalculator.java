...
extends ForwardBackwardCalculator
{
			...
			Hmm<O> hmm, EnumSet<Computation> flags)
	{
		...
		if (Arguments.DEBUG.getIsSet())
		{
			System.out.printf("Computed Alpha:\n");
			for (int i = 0; i < alpha.length; ++i)
			{
				System.out.printf("[t=%d][", i);
				for (int j = 0; j < alpha[i].length - 1; ++j)
					System.out.printf("%f,\t", alpha[i][j]);
				System.out.printf("%f]\n", alpha[i][alpha[i].length - 1]);
			}
		}
	...
	{	
		...
		// skip over the first obs
		// the if is really not required since we know that the list is not empty
		// note java iterators are diff from C++ iterators since they don't actually
		// point to the element but rather the spaces between elements
		// see: http://docs.oracle.com/javase/7/docs/api/java/util/ListIterator.html
