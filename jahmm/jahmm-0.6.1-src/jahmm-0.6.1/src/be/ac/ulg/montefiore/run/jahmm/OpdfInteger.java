...
{	
	...
	public ObservationInteger generate(Integer state)
	{	
		double rand = Math.random();
		
		for (int i = 0; i < probabilities.length - 1; i++)
			if ((rand -= probabilities[i]) < 0.)
				return new ObservationInteger(i, state);
		
		return new ObservationInteger(probabilities.length - 1, state);
	}
	...
	{	
		...
		for (int i = 0; i < nbEntries();) {
			ObservationInteger oi = new ObservationInteger(i, DummyState);
