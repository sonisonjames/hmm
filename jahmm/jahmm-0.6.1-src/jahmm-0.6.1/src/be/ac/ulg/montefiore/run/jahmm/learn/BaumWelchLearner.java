...
{	
	...
	/* alpha[seq-n][t][i] = P(O(1), O(2),..., O(t+1), i(t+1) = i+1 | hmm), that is the
	...
	public double[][] getAlpha(int indx) {
		return alpha[indx];
	}

	public void setAlpha(int indx, double[][] alpha) {
		this.alpha[indx] = alpha;
	}
	...
	public double[][] getBeta(int indx) {
		return beta[indx];
	}

	public void setBeta(int indx, double[][] beta) {
		this.beta[indx] = beta;
	}
	...
	{		
		...
		for (List<? extends O> obsSeq : sequences) {	    
			...
			setAlpha(g, fbc.getAlpha());
			setBeta(g, fbc.getBeta());
		...
		for (int i = 0; i < hmm.nbStates(); i++) {
			if (aijDen[i] == 0.) // State i is not reachable
				for (int j = 0; j < hmm.nbStates(); j++)
					// TODO, should this not be set to zero?
