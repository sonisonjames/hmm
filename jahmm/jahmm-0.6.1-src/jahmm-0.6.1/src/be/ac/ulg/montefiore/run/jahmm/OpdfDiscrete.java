...
{		
	...
	{		
		...
		for (E value : values) 
			result.put(value, new ObservationInteger(value.ordinal(), DummyState));
	...
	public ObservationDiscrete<E> generate(Integer state)
	{
		return 
		new ObservationDiscrete<E>(values.get(distribution.generate(state).value), state);
	}
	...
	public String toString(NumberFormat numberFormat)
	{
		...
		for (int i = 0; i < values.size();) {
			ObservationDiscrete o = new ObservationDiscrete<E>(values.get(i), DummyState);
